...
function GamePageClient() {
  ...
  // Add state to track right-click
  const [handlingRightClick, setHandlingRightClick] = useState(false);
  ...
  */
  ...
  const handleKanjiClick = (kanji: string, event: React.MouseEvent | React.TouchEvent) => {
    ...
    setHandlingRightClick(true);
    
    // Reset the flag after a short delay
    setTimeout(() => {
      setHandlingRightClick(false);
    }, 300);
    
    // Make sure we're not in any drag state
    setIsDraggingFromSidebar(false);
    ...
    // No need to add the kanji to the workspace here
  };
        ...
        >
          ...
          {elements.filter(el => el.position.x !== 0 || el.position.y !== 0).map((element) => (
            <div
              ...
              onTouchStart={(e) => {
                // Skip if we're handling a right-click equivalent on touch
                if (handlingRightClick) return;
